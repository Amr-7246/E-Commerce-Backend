...
import { IStudent, Student } from "../students/models/StudentsModel";
...
// ~ Send JWT and refresh token in the response
  const sendResponse = async (res: Response, user: any, code: number): Promise<void> => {
    ...
    const updated = await Student.findByIdAndUpdate(user._id , { refreshToken });
...
// ~ Protect middleware
  ...
  export const protect = catchError(
    async (
      req: Request extends { user: IStudent } ? Request & { user: IStudent } : Request,
      res: Response,
      next: NextFunction
    ): Promise<void> => {
      let token = "";
...
// ~ Refresh token handler
  export const refresh = catchError(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    jwt.verify(refreshToken, process.env.REFRESH_TOKEN as string, async (err: any, decoded: any) => {
      ...
      console.log(decoded);
      const existingUser: IStudent | null = await Student.findById(decoded.id);
