...
const catchError_1 = require("@src/utils/catchError");
const StudentsModel_1 = require("../students/models/StudentsModel");
...
const sendResponse = async (res, user, code) => {
    ...
    const updated = await StudentsModel_1.Student.findByIdAndUpdate(user._id, { refreshToken });
...
exports.register = (0, catchError_1.catchError)(async (req, res, next) => {
    const newUser = await StudentsModel_1.Student.create(Object.assign({}, req.body));
...
exports.login = (0, catchError_1.catchError)(async (req, res, next) => {
    ...
    const user = await StudentsModel_1.Student.findOne({ email }).select("+password");
...
exports.protect = (0, catchError_1.catchError)(async (req, res, next) => {
    ...
    const currentUser = await StudentsModel_1.Student.findById(decoded.id);
    console.log(decoded);
    if (!currentUser)
...
exports.refresh = (0, catchError_1.catchError)(async (req, res, next) => {
    ...
    jsonwebtoken_1.default.verify(refreshToken, process.env.REFRESH_TOKEN, async (err, decoded) => {
        ...
        console.log(decoded);
        const existingUser = await StudentsModel_1.Student.findById(decoded.id);
...
exports.logout = (0, catchError_1.catchError)(async (req, res, next) => {
    ...
    const refreshToken = req.cookies.jwt;
    const user = await StudentsModel_1.Student.findOne({ refreshToken });
    if (!user) {
